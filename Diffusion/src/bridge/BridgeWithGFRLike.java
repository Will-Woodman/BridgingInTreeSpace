...
/** Implementation of the forward bridge using the GFR likelihood - not recommended to use as GFR
 * is a poor choice and leads to the MCMC sampler getting stuck at boundaries
 */

public class BridgeWithGFRLike extends ForwardStepBridge {
    
    public BridgeWithGFRLike(TreeAsSplits tA, TreeAsSplits tB, int n) {
        super(tA, tB, n);
    }
    ...
    public ForwardStepBridge makeEmptyCopy() {
        return new BridgeWithGFRLike(treeA, treeB, numSteps);
    }
    @Override
    public ForwardStepBridge makeFreshBridge(TreeAsSplits tA, TreeAsSplits tB) {
        return new BridgeWithGFRLike(tA, tB, numSteps);
    }
    @Override
    public ForwardStepBridge makeEmptyCopy(TreeAsSplits x, boolean whichEnd) {
        if (whichEnd) return new BridgeWithGFRLike(treeA, x, numSteps);
        else return new BridgeWithGFRLike(x, treeB, numSteps);
    }
        ...
        while (it.hasNext()) {
            ...
            if (p.isTerminal()==null) {
                ...
                if (!tB.contains(p)) {
                    ...
                    //GFR likelihood
                    ll += NormalDistribution.logpdf(la+lb, 0.0, likeStepSD)-Math.log(3.0);
                    ...
                    //GFR likelihood
                    ll += Math.log(NormalDistribution.pdf(la-lb, 0.0, likeStepSD)+NormalDistribution.pdf(la+lb, 0.0, likeStepSD)/3.0);
