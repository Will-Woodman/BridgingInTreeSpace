...
import MCMC.PositiveParameter.PositiveParameterProposal;
...
public class TunnelSamplingProposal {
    ...
    /*
     Run the MCMC targetting the proposal distribution on t0 and bridges when calculating marginal likelihoods
     ...
     Proposal distribution is the reference dist on t0 multiplied by independence proposal on each of the bridges
        */    
    public static void simConditionalProposal(String[] args) throws AlgorithmException {
       ...
       //get the independence proposal rho on t0
       if(mu==0){//then we have not passed in a parametrised reference distribution and use the following:
       System.out.println("No parametrised ref dist passed to Indep prop MCMC sampler: it is advised to parametrise a ref dist");    
       ...
       DensityCalculator thePrior = new BrownianStateForBridgingTunnel.TunnelBrownianStatePrior(t0Prior,t0Dist,true);
       ...
       System.out.println("Reference: Lognormal with mu = "+String.format("%7.7f", mu)+" and sigma = "+String.format("%7.7f", sigmaRho));
        ...
        ForwardStepBridge template = new BridgeWithIPLike(x0, null, numSteps);//Independence proposal likelihood given simple steps
        try {
            for(int i=0; i<2; i++) {
                BrownianStateForBridgingTunnel state = new BrownianStateForBridgingTunnel(x0, t0, theData.theTrees, (i==0), template,t0Prior,t0Dist,true);
