...
/**
 * HashMap of compatible splits to Double.
 */
...
public class TreeAsSplits implements java.io.Serializable {
    ...
    /** Constructor from a file of splits -- added by Will 20230911*/
    
    public TreeAsSplits(File theFile) throws AlgorithmException {
        HashMap<Split,Double> Splits = new HashMap();
        ArrayList<String> Taxas = new ArrayList();
        ArrayList<String[]> TaxaBySplit = new ArrayList();
        //extract the names of all the taxa
        int start;
        int end;
        String taxa;
        String rest;
        ArrayList<Double> Lengths = new ArrayList();
        //build the splits from the names
        ...
        try {
            ...
            while ((s = br.readLine()) != null) {
                ...
                taxa = s.substring(start+1,end);
                System.out.println(taxa);
                rest = s.substring(end+2);
                Lengths.add(Double.parseDouble(rest));
                TaxaBySplit.add(taxa.split(", "));
                if (taxa.split(", ").length == 1) Taxas.add(taxa);
            }
        } catch (java.io.IOException ioex) {
            throw new AlgorithmException("Error reading newick file.");
        }
        
        //for each split I now what to create a new set the contains all of the other taxa
        HashSet<String> splitsA;
        HashSet<String> splitsB;
        Split theSplit;
        int added =0;
        
        int i=0;
        for(String[] ts:TaxaBySplit){
            ...
            for(String t:Taxas){
                ...
                if(added==0) splitsB.add(t);
            }
            System.out.println(splitsA);
            System.out.println(splitsB);
            theSplit = new Split(splitsA,splitsB);
            Splits.put(theSplit, Lengths.get(i));
            i=i+1;
        }
        theMap = Splits;
        Split p = Splits.keySet().iterator().next();
        numTaxa = p.numTaxa();
        checkNaNs();
    }
    ...
    public HashSet<Split> getTrivialSplits() {
        HashSet<Split> s = new HashSet();
        Iterator<Split> it = theMap.keySet().iterator();
        for (int i=0; i<theMap.size(); i++) {
            Split p = it.next();
    ...
    public double sumSquaredLengths(boolean ignorePendants) {
        ...
        public double sumLengths(HashSet<Split> ignore) {
        double s = 0.0, e;
        Iterator<Split> it = theMap.keySet().iterator();
        for (int i=0; i<theMap.size(); i++) {
            Split p = it.next();
    ...
    public double sumLengths(boolean ignorePendants) {
        double s = 0.0, e;
        Iterator<Split> it = theMap.keySet().iterator();
        for (int i=0; i<theMap.size(); i++) {
            Split p = it.next();
    ...
    public double sumSquaredShortestTwoLengths(boolean ignorePendants) {
        if(numTaxa<=5) return(sumSquaredLengths(ignorePendants));
        else{
        double s = 0.0, e;
        double[] t={0.0,0.0};
        Iterator<Split> it = theMap.keySet().iterator();
        //get the first two non-trivial splits
        int k=0;
        int i=0;
        while(i <2){
            Split p = it.next();
             if (p.isTerminal()==null) {
             t[i]=getSplitLength(p);
             i=i+1;
            }
             k=k+1;
        }
        for (int j=k; j<theMap.size(); j++) {
            Split p = it.next();
            if (p.isTerminal()==null) {
                e = getSplitLength(p);
                int Pos = (e<t[0])? 0:2;
                if(Pos==2) Pos = (e<t[1])? 1:2;
                if(Pos!= 2) t[Pos]=e;
            }
        }
